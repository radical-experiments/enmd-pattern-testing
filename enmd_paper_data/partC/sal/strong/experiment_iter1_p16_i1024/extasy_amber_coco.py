...
class Extasy_CocoAmber_Static(SimulationAnalysisLoop):
    ...
    def analysis_step(self, iteration, instance):
        ...
        num_of_ana_inst = int(Kconfig.num_CUs)/16
	for iter in range(1,iteration+1):
#            for i in range(1,Kconfig.num_CUs+1):
	     for i in range(1+(instance-1)*16, instance*16 + 1):
        ...
        for i in range(0,16):
            k1.copy_output_data = k1.copy_output_data + ['pdbs{1}.pdb > $PRE_LOOP/pentaopt{0}{2}.pdb'.format(iteration,i,(instance-1)*16+i)]

        if(iteration%Kconfig.nsave==0):
            k1.download_output_data = ['coco.log > output/iter{0}/coco.log'.format(iteration,instance)]


        k2 = Kernel(name="md.tleap",instance_type='single')
...
if __name__ == "__main__":
    ...
    try:
        ...
        coco_amber_static = Extasy_CocoAmber_Static(maxiterations=Kconfig.num_iterations, simulation_instances=Kconfig.num_CUs, analysis_instances=Kconfig.num_CUs/16)
